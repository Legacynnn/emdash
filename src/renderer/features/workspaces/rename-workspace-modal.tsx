...
type RenameWorkspaceModalArgs = {
  projectId: string;
  workspaceId: string;
  currentName: string;
};

type Props = BaseModalProps<void> & RenameWorkspaceModalArgs;

export const RenameWorkspaceModal = observer(function RenameWorkspaceModal({
  projectId,
  workspaceId,
  currentName,
  onSuccess,
  ...
  const siblingNames = new Set(
    Array.from(taskManager?.tasks.values() ?? [])
      .filter((t) => t.state !== 'unregistered' && t.data.id !== workspaceId)
      .map((t) => t.data.name)
  );
  ...
  const handleSubmit = useCallback(async () => {
    if (!isValid) return;
    const task = taskManager?.tasks.get(workspaceId);
    if (!task) return;
  ...
  }, [isValid, taskManager, workspaceId, normalizedName, onSuccess]);
