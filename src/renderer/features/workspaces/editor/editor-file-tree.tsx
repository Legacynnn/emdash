...
import { buildVisibleRows } from '@renderer/features/workspaces/editor/stores/files-store-utils';
import {
  useWorkspace,
...
const FileTreeRow = observer(function FileTreeRow({
  ...
  const editorView = workspaceView.editorView;

  const isExpanded = editorView.expandedPaths.has(node.path);
  const isSelected = workspaceView.tabManager.activeFilePath === node.path;
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (node.type === 'directory') {
      ...
      workspaceView.tabManager.openFilePreview(node.path);
    }
  };
  ...
  const handleDoubleClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (node.type === 'file') {
      workspaceView.tabManager.openFile(node.path);
    }
  };
