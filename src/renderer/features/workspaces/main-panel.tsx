...
import { WorkspaceSidebar } from './view/workspace-sidebar';
...
const ReadyTaskMainPanel = observer(function ReadyTaskMainPanel() {
  ...
  useEffect(() => {
    ...
    if (workspaceView.isSidebarCollapsed) {
      sidebarPanelRef.current?.collapse();
    } else {
      sidebarPanelRef.current?.expand();
    }
  }, [workspaceView.isSidebarCollapsed, sidebarPanelRef]);
  ...
  return (
    <ResizablePanelGroup orientation="horizontal" id="task-sidebar-layout">
      ...
      <ResizablePanel
        ...
        onResize={() =>
          workspaceView.setSidebarCollapsed(sidebarPanelRef.current?.isCollapsed() ?? false)
        }
      >
        <WorkspaceSidebar />
      </ResizablePanel>
...
const TaskMainColumn = observer(function TaskMainColumn() {
  ...
  useEffect(() => {
    panelDragStore.suppressFor(140);
    if (workspaceView.isTerminalDrawerOpen) {
      bottomPanelRef.current?.expand();
    } else {
      bottomPanelRef.current?.collapse();
    }
  }, [workspaceView.isTerminalDrawerOpen, bottomPanelRef]);
  ...
  return (
    <ResizablePanelGroup orientation="vertical" id="task-main-vertical">
      ...
      <DraggableResizeHandle className={workspaceView.isTerminalDrawerOpen ? 'flex' : 'hidden'} />
      <ResizablePanel
        ...
        onResize={(_panelSize, _id, prevPanelSize) => {
          ...
          workspaceView.setTerminalDrawerOpen(!bottomPanelRef.current?.isCollapsed());
        }}
      >
        <TerminalsPanel />
      </ResizablePanel>
...
const UnifiedMainContent = observer(function UnifiedMainContent() {
  const { projectId, workspaceId } = useTaskViewContext();
  const workspaceView = useWorkspaceViewModel();
  const { tabManager } = workspaceView;
  const { setEditorHost, triggerLayout } = useEditorContext();
  const showCreateConversationModal = useShowModal('createConversationModal');

  const renderer = workspaceView.activeRenderer;
