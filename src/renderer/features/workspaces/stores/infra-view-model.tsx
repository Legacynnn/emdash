...
import type { DiffViewSnapshot, WorkspaceViewSnapshot } from '@shared/view-state';
import type { Workspace } from '@shared/workspaces';
import { DiffTabLifecycleStore } from '@renderer/features/workspaces/diff-view/stores/diff-tab-lifecycle-store';
import { DiffViewStore } from '@renderer/features/workspaces/diff-view/stores/diff-view-store';
import { FileModelLifecycleStore } from '@renderer/features/workspaces/editor/stores/file-model-lifecycle-store';
import { DevServerStore } from '@renderer/features/workspaces/stores/dev-server-store';
import { TabManagerStore } from '@renderer/features/workspaces/tabs/tab-manager-store';
import { TerminalTabViewStore } from '@renderer/features/workspaces/terminals/terminal-tab-view-store';
import { type SidebarTab } from '@renderer/features/workspaces/types';
...
export class WorkspaceViewModel implements ILifecycle {
  ...
  constructor(private readonly _taskStore: WorkspaceStore) {
    const taskData = _taskStore.data as Workspace;
    ...
    this.tabManager = new TabManagerStore(
      () => conversationRegistry.get(this.workspaceId) ?? null,
      workspaceId
    );
    this.terminalTabs = new TerminalTabViewStore(
      () => terminalRegistry.get(this.workspaceId) ?? null
    );
    ...
    const initConvDisposer = reaction(
      () => conversationRegistry.get(this.workspaceId)?.conversations.size ?? 0,
      (size) => {
      ...
      reaction(
        ...
        (tabId) => {
          ...
          appState.history.push({
            kind: 'tab',
            projectId: (this._taskStore.data as Workspace).projectId,
  ...
  private get _workspace() {
    ...
    const projectId = (this._taskStore.data as Workspace).projectId;
    return infraRegistry.get(projectId, workspaceId) ?? null;
  }
  ...
  get snapshot(): WorkspaceViewSnapshot {
    return {
   ...
   */
  restoreSnapshot(savedSnapshot: WorkspaceViewSnapshot): void {
  ...
  initialize(): void {
    ...
    const taskData = this._taskStore.data as Workspace;
    const workspaceId = this._taskStore.workspaceId!;
    this.devServers = new DevServerStore(this.workspaceId, workspaceId);
    ...
    this._snapshotDisposer = snapshotRegistry.register(
      `task:${this.workspaceId}`,
      () => this.snapshot
    );
    ...
    const terminalsDisposer = reaction(
      () => {
        const terminals = terminalRegistry.get(this.workspaceId);
        return (
      ...
      (shouldCreate) => {
        if (shouldCreate) void terminalRegistry.get(this.workspaceId)?.createDefaultTerminal();
      }
    );
  ...
  dispose(): void {
    this.suspend();
    appState.history.prune((e) => e.kind === 'tab' && e.workspaceId === this.workspaceId);
  ...
  openNewTerminal(): void {
    ...
    void terminalRegistry.get(this.workspaceId)?.createDefaultTerminal();
  }
}
