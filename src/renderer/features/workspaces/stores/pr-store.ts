...
import { isRegistered, type WorkspaceStore } from './workspace-store';
...
export class PrStore {
  ...
  constructor(
    ...
    private readonly taskStore: WorkspaceStore
  ) {
  ...
  get pullRequests(): PullRequest[] {
    ...
    return (this.taskStore.data as Workspace).prs ?? [];
  }
   ...
   * `prUpdatedChannel` and be merged into `task.data.prs` by `WorkspaceManagerStore`.
   */
