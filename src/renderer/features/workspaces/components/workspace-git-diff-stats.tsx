...
export function useWorkspaceGitDiffStats(task: WorkspaceStore): {
  linesAdded: number;
  linesDeleted: number;
  visible: boolean;
} {
 ...
 */
export const WorkspaceGitDiffStats = observer(function WorkspaceGitDiffStats({
  task,
  className,
}: {
  task: WorkspaceStore;
  ...
  const { linesAdded, linesDeleted, visible } = useWorkspaceGitDiffStats(task);
