...
export class TerminalManagerStore implements IDisposable {
  ...
  constructor(projectId: string, workspaceId: string) {
    ...
    this.list = new Resource<Terminal[]>(
      () => rpc.terminals.getTerminalsForWorkspace(projectId, workspaceId),
      [{ kind: 'demand' }]
    );
          ...
          for (const terminal of data) {
            ...
            if (!this.sessions.has(terminal.id)) {
              this.sessions.set(
                terminal.id,
                new PtySession(
                  makePtySessionId(terminal.projectId, terminal.workspaceId, terminal.id)
                )
              );
            }
          }
  ...
  async createTerminal(params: CreateTerminalParams): Promise<Terminal> {
    ...
    runInAction(() => {
      ...
      this.sessions.set(
        params.id,
        new PtySession(makePtySessionId(params.projectId, params.workspaceId, params.id))
      );
    });
