...
export const WorkspaceTitlebar = observer(function WorkspaceTitlebar() {
  ...
  if (kind !== 'ready') {
    return <PendingWorkspaceTitlebar workspaceId={workspaceId} projectId={projectId} />;
  }

  return <ActiveWorkspaceTitlebar workspaceId={workspaceId} projectId={projectId} />;
});

const PendingWorkspaceTitlebar = observer(function PendingWorkspaceTitlebar({
  workspaceId,
  projectId,
}: {
  workspaceId: string;
...
const ActiveWorkspaceTitlebar = observer(function ActiveWorkspaceTitlebar({
  projectId,
  workspaceId,
}: {
  projectId: string;
  ...
  const taskStore = getWorkspaceStore(projectId, workspaceId)!;
  const taskPayload = getRegisteredTaskData(projectId, workspaceId)!;
  ...
  const {
    ...
    isPulling,
    isPushing,
  } = useGitActions(projectId, workspaceId);
    ...
    <Titlebar
      ...
      rightSlot={
        <div className="flex items-center gap-2">
          <DevServerPills projectId={projectId} workspaceId={workspaceId} />
          {!isRemoteProject && (
          ...
          <Tooltip>
            <TooltipTrigger>
              <Toggle
                size="sm"
                pressed={workspaceView.isTerminalDrawerOpen}
          ...
          <Separator orientation="vertical" className="h-5 self-center!" />
          <ToggleGroup
            value={workspaceView.isSidebarCollapsed ? [] : [workspaceView.sidebarTab]}
            onValueChange={([tab]) => {
              if (!tab) {
                workspaceView.setSidebarCollapsed(true);
              } else {
                workspaceView.setSidebarTab(tab as SidebarTab);
