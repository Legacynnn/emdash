...
export const CommitCard = observer(function CommitCard({ autoStage = false }: CommitCardProps) {
  ...
  const hasOpenPr = workspaceView.prStore?.pullRequests.some((p) => p.status === 'open') ?? false;
  const canCreatePr =
    Boolean(workspace.repository.repositoryUrl) && Boolean(taskData?.workspaceBranch) && !hasOpenPr;
  ...
  const doCommitAndCreatePr = async () => {
    ...
    showCreatePrModal({
      ...
      branchName: taskData?.workspaceBranch ?? '',
      draft: false,
      onSuccess: () => {},
    });
  };
