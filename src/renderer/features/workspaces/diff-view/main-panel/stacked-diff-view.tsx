...
import {
  ...
  type DiffSlotStore,
} from '@renderer/features/workspaces/diff-view/stores/stacked-diff-panel-store';
import {
  useInfraId,
  useTaskViewContext,
  useWorkspace,
...
export const StackedDiffView = observer(function StackedDiffView() {
  ...
  const pr = workspaceView.prStore;

  const panelStore = useMemo(
    () => (diffView ? new StackedDiffPanelStore(projectId, infraId, diffView, git, pr!) : null),
