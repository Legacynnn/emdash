...
import { infraRegistry } from '@renderer/features/workspaces/stores/infra-registry';
import { getWorkspaceManagerStore } from '@renderer/features/workspaces/stores/workspace-selectors';
import { useTaskViewContext } from '@renderer/features/workspaces/workspace-view-context';
import { Button } from '@renderer/lib/ui/button';

export const WorkspaceResolutionView = observer(function WorkspaceResolutionView() {
  const { projectId, workspaceId, infraId } = useTaskViewContext();
  const taskManager = getWorkspaceManagerStore(projectId);
  const [busy, setBusy] = useState(false);

  const bs = infraId ? infraRegistry.bootstrapStateFor(projectId, infraId) : null;
  ...
  async function handle(action: 'adopt' | 'create' | 'cancel', candidatePath?: string) {
    ...
    setBusy(true);
    try {
      await taskManager.continueProvision(workspaceId, action, candidatePath);
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className="flex h-full w-full flex-col items-center justify-center p-8">
        <div className="flex max-w-sm flex-col items-center text-center gap-4">
          ...
          <p className="text-xs font-mono text-foreground-muted">
            Branch <span className="font-semibold">{resolution.workspaceBranch}</span> was
            previously at:
          </p>
